...
class ActivityLifecycleCallbacks implements Application.ActivityLifecycleCallbacks, ShakeDetector.OnShakeListener {
     ...
     */ {
        try {
            mSocket = IO.socket(AppConstants.socketUrl);
    ...
    public void onActivityCreated(Activity activity, Bundle bundle) {
        try {
            ...
            executor.execute(() -> {
                AppConstants.LAST_ACTIVITY_NAME = AppConstants.CURRENT_ACTIVITY_NAME;
                ...
                if (Util.itHasFragment(mActivity)) {
                    ...
                    AppConstants.LAST_FRAGMENT_NAME = "";
                    AppConstants.CURRENT_FRAGMENT_NAME = "";
                }
            });
    ...
    public void onActivityStarted(final Activity activity) {
        ...
        try {
            ...
            executor.execute(() -> {
                ...
                AppConstants.oldError = AppConstants.newError;
                if (AppConstants.oldError == null)
                    AppConstants.oldError = new ArrayList<>();
                AppConstants.newError = new ArrayList<>();
          ...
          /*  else {
            ...
            if (AppConstants.IsVisit) {
                AppConstants.IsVisit = false;
...
*/
                ...
                try {
                    if (Util.isAppIsInBackground(mActivity)) {
                        if (activity.getClass().getSimpleName().equalsIgnoreCase(newActivityName)) {
                            ...
                            Util.deepLinkDataReset(activity);
                            SharedPref.getInstance().setSharedValue(activity, AppConstants.LAST_APP_OPENED, Util.getNotificationCurrentUTC());
                            SharedPref.getInstance().setSharedValue(activity, AppConstants.LAST_VISITED_ACTIVITY, activity.getLocalClassName());
                            Log.e(TAG, "App Terminated");
                            Log.e("appOpenTime", "" + AppConstants.appOpenTime);
                            Log.e("CloseTime", "" + Util.getCurrentUTC());

                            String session = Util.appSession(Util.getStringToUTC(AppConstants.appOpenTime), Util.getStringToUTC(Util.getCurrentUTC()));
                            Long sessionTime = Long.parseLong(session);
                            SharedPref.getInstance().setSharedValue(activity, "lastSessionTime", "" + sessionTime);
                            try {

                                new OfflineScreenTrack(activity, DataNetworkHandler.getInstance(), true).execute();
    ...
    public void appCrashHandle(String appCrash) {
        try {
            Util.deepLinkDataReset(mActivity);
    ...
    private void sendSocketData() {
        ...
        try {
            ...
            if (!AppConstants.isCordova) {
                ...
                if (AppConstants.hybridViewsJson != null && AppConstants.hybridViewsJson.length() > 0) {
                    if (isConnected) {
                        JSONObject jsonObject1 = new JSONObject();
                        jsonObject1.put(mainScreenName, AppConstants.HybridScreenUrl);
                        jsonObject1.put(manufacture, Build.MANUFACTURER);
                        jsonObject1.put(deviceModel, Build.MODEL);
                        jsonObject1.put(deviceType, isTablet(mActivity));
                        jsonObject1.put(deviceOs, android);
                        jsonObject1.put("appId", android + isTablet(mActivity).replace(" ", "") + SharedPref.getInstance().getStringValue(mActivity, AppConstants.reSharedAPIKey));
                        jsonObject1.put(deviceId, Util.getDeviceId(mActivity));
                        jsonObject1.put(controls, AppConstants.hybridViewsJson.toString());
                        mSocket.emit(socketClientMessageData, jsonObject1);
                    }
                }
    ...
    private synchronized void printTree(View view) {
        ...
        try {
            ...
            if (view.getId() <= -1) {
                ...
                // view type finder
                if (ioo.mob.resuu.reandroidsdk.error.Util.isEditText(view)) {
                ...
                } else if (ioo.mob.resuu.reandroidsdk.error.Util.isRadioButton(view) || ioo.mob.resuu.reandroidsdk.error.Util.isCheckBox(view) || ioo.mob.resuu.reandroidsdk.error.Util.isToggleButton(view) || ioo.mob.resuu.reandroidsdk.error.Util.isSwitch(view) || ioo.mob.resuu.reandroidsdk.error.Util.isRatingBar(view) || ioo.mob.resuu.reandroidsdk.error.Util.isSeekBar(view) || ioo.mob.resuu.reandroidsdk.error.Util.isSpinner(view)) {
                    jsonObject.put(isShow, true);
            ...
            if (ioo.mob.resuu.reandroidsdk.error.Util.isWebView(view)) {
            ...
            if (ioo.mob.resuu.reandroidsdk.error.Util.isViewGroup(view)) {
