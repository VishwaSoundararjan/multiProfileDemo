...
public class OfflineScreenTrack /*extends AsyncTask<String, String, String>*/ {
    ...
    OfflineScreenTrack(Context context, APIInterface apiInterface, boolean appClose) {
        this.context = context;
    ...
    private void onPostExecute(String screenObject) {
        try {
            if (dbScreen.size() > 0)
        ...
        try {
         ...
         /*   try {
            ...
            if (AppConstants.deviceData == null) {
                ...
                try {
                    if (AppConstants.lastDeviceData != null) {
                        ...
                        while (stringSet.hasNext()) {
                            ...
                            String value2 = AppConstants.lastDeviceData.optString(key, "");
            ...
            Long sessionTime = Long.parseLong(Util.appSession(Util.getStringToUTC(AppConstants.appOpenTime), Util.getStringToUTC(Util.getCurrentUTC())));
            Long screenTime = Long.parseLong(Util.appSession(Util.getStringToUTC(AppConstants.pageStartTime), Util.getStringToUTC(Util.getCurrentUTC())));
            ...
            journeyAttribute.put("fragmentName", AppConstants.CURRENT_FRAGMENT_NAME);
            ...
            journeyAttribute.put("LastCustomEvent", AppConstants.APP_CURRENT_EVENT);
            journeyAttribute.put("token", SharedPref.getInstance().getStringValue(context, "token"));
            journeyAttribute.put("appOpenedDate", Util.getStringToUTC(AppConstants.appOpenTime));
            ...
            journeyAttribute.put("login", AppConstants.isLogin);
            ...
            try {
                ...
                if (AppConstants.lastJourneyData != null) {
                    Iterator<String> stringSet = journeyAttribute.keys();
                    while (stringSet.hasNext()) {
                        String key = stringSet.next();
                        String value1 = journeyAttribute.optString(key, "");
                        String value2 = AppConstants.lastJourneyData.optString(key, "");
                        if (!value1.equalsIgnoreCase(value2)) {
                            if (!TextUtils.isEmpty(value1))
            ...
            AppConstants.lastJourneyData = journeyAttribute;
            // Resume journey data
            ...
            resumeJourney.put("activityName", AppConstants.CURRENT_ACTIVITY_NAME);
            resumeJourney.put("fragmentName", AppConstants.CURRENT_FRAGMENT_NAME);
